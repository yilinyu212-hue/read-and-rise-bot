...
def run():
    ...
    MODEL_LIST = ["MECE", "First Principles"]
    ...
    for src in SOURCES:
        try:
            ...
            if feed.entries:
                e = feed.entries[0]
                p = f"针对《{e.title}》输出讲义JSON: {{'level':'C1','en_excerpt':'原文','cn_translation':'翻译','vocabulary_pro':'词汇','syntax_analysis':'句法','output_playbook':{{'speaking':'口语','writing':'写作'}},'insight':'洞察','mindmap':'Mermaid格式的思维导图代码，根节点用文章标题'}}"
                ai = get_ai_data(p)
                if ai:
    ...
    # 3. 思维模型引擎（增加思维导图逻辑）
    for m in MODEL_LIST:
        p = f"解析模型《{m}》。JSON: {{'name':'Name (中文名)','definition':'Eng (中)','how_to_use':'Eng (中)','english_template':['S1','S2','S3'],'mindmap':'Mermaid格式的思维导图代码'}}"
        ai_m = get_ai_data(p)
